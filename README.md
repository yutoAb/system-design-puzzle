...
- お題が与えられる（例:「最大50万人が同時視聴するライブ授業配信を設計せよ」）
- AI面接官と音声で会話しながら、ホワイトボードにコンポーネント（CDN、ロードバランサー、DB、キャッシュなど）を配置して設計を説明する
- 面接は「①要件・スコープ確認 → ②概要設計 → ③深掘り → ④まとめ」の4ステップで進行（実際のシステム設計面接で一般的な進め方）
- 終了後、会話の書き起こしと最終的なボードをもとにAIが評価レポート（総合評価・良かった点・改善点・概念カバレッジ）を生成

## 題材

- オンライン予備校のライブ授業配信（最大50万人同時視聴）
- スマホゲームのフレンド＆ギルドチャット（20万常時接続）
- レシピ投稿アプリのフォローフィード（読み込み:書き込み = 200:1）

お題・要件・模範解答はすべて本サービスのオリジナルコンテンツです。
